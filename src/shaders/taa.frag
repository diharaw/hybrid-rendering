...
layout (location = 0) out vec3 FS_OUT_Buffer;
layout (location = 1) out vec3 FS_OUT_Screen;
...
layout(set = 0, binding = 0) uniform sampler2D s_Current;
layout(set = 0, binding = 1) uniform sampler2D s_Prev;

layout(set = 1, binding = 2) uniform sampler2D s_Velocity;
...
void main()
{
	...
	vec2 uv = FS_IN_TexCoord - u_CurrentPrevJitter.xy;
#else
	...
	float vs_dist = texture(s_Depth, uv);
#endif
	// temporal resolve
	...
	// add noise
	vec4 noise4 = srand4(FS_IN_TexCoord + u_TimeParams.y + 0.6959174) / 510.0;
